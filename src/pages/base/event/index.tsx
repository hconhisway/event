...
export interface EventPageProps {
  ...
  setCompetitionEvent: (event?: EventEditModel) => void;
...
class EventPage extends React.Component<EventPageProps> {
...

    {
      ...
      render: (_: any, record: EventModel) => (
        <>
          <a
            onClick={() => {
              this.props.setCompetitionEvent({
                ...record,
              });
    ...
    const buttons: ButtonItem[] = [
      {
        ...
        type: 'default',
        onClick: () => {
          this.props.setCompetitionEvent(undefined);
          this.props.setVisible(true);
        },
      },
      {
    ...
    return (
      <>
        ...
        <React.Suspense fallback={<Loading />}>
          <CommonModal
            title={!!this.props.event ? '修改项目' : '新增项目'}
  ...
  private handleDelete = (codeList: string[]) => {
    ...
    const modal = Modal.confirm({
      centered: true,
      title: `您确定要删除选定的${codeList.length}个项目吗？`,
      okText: '确定',
      cancelText: '取消',
      onOk: async () => {
        modal.update({
...
export default compose(
  ...
  withState('event', 'setCompetitionEvent', undefined),
